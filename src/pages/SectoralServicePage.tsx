...
import { getPageBanner, getSectoralBySlug, getSectoralHeroImage } from '../content/selectors'
...
export function SectoralServicePage() {
  const { content } = useSiteContent()
  const { slug } = useParams<{ slug: string }>()
  const { pathname } = useLocation()
  const service = slug ? getSectoralBySlug(content, slug) : undefined

  if (!service) {
    return <Navigate to="/sektorel" replace />
  }

  const heroImage = getSectoralHeroImage(content, service.slug)
  const pageBanner = getPageBanner(content, 'sektorelDetay', heroImage)
  const related = content.sectoralServices.filter((s) => s.slug !== service.slug).slice(0, 4)
  const { site } = content
  const pageTitle = `${service.code} — ${service.title} | ${site.name}`

  const bc = [
    { name: 'Ana sayfa', path: '/' },
    { name: 'Sektörel hizmetler', path: '/sektorel' },
  ...
  const tocIds = useMemo(() => ['giris', 'hizmet-kapsami', 'benzersiz', 'surec'], [])
  ...
  const L = {
    ...
    process: 'Tipik süreç',
    unique: service.customSectionTitle?.trim() || 'Sektöre özel değer',
    uniqueEmpty: 'Bu bölüm için içerik admin panelinden eklenebilir.',
    processNote: 'Projeye göre süre ve adımlar uyarlanabilir.',
    ctaTitle: `${service.code} için teklif alın`,
    ctaSub: 'Kapsam ve süreyi birlikte netleştirelim.',
    ctaBtn: 'İletişim formu',
    back: '← Tüm sektörel hizmetler',
    ...
    seeAll: 'Tümünü gör',
    tocSummary: 'Özet',
    tocScope: 'Hizmet kapsamı',
    tocUnique: service.customSectionTitle?.trim() || 'Sektöre özel değer',
      ...
      <section className="relative min-h-[min(56vh,28rem)] overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex min-h-[min(56vh,28rem)] max-w-6xl flex-col justify-end px-4 pb-12 pt-28 sm:px-6 sm:pb-16 sm:pt-32 lg:px-8">
          <nav className="mb-6 text-sm text-white/75" aria-label={L.bc}>
            ...
            <Link to="/sektorel" className="transition hover:text-white">
              Sektörel hizmetler
            </Link>
            <span className="mx-2 text-white/40" aria-hidden>
              /
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-12 md:grid-cols-[minmax(0,1fr)_16rem] md:gap-10 lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-14 xl:grid-cols-[minmax(0,1fr)_20rem]">
            <div className="min-w-0">
              ...
              <FadeIn>
                <section className="mt-10 scroll-mt-28" id="hizmet-kapsami" aria-labelledby="hizmet-kapsami-baslik">
                  <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-4">
                    <h2 id="hizmet-kapsami-baslik" className="font-[family-name:var(--font-display)] text-2xl font-bold text-slate-900">
                      {L.scope}
                    </h2>
                  </div>
                  <SectionImage
                    src={service.offeringsImageUrl}
                    alt={`${service.code} ${service.title} hizmet kapsami gorseli`}
                  />
              ...
              <FadeIn>
                <div className="mt-10 flex flex-col gap-4 border-t border-slate-200 pt-10 sm:flex-row sm:items-center sm:justify-between">
                  <Link to="/sektorel" className="text-sm font-semibold text-primary-700 hover:underline">
                    {L.back}
                  </Link>
                  <Link
                    to="/iletisim"
            ...
            <aside className="md:sticky md:top-28 md:self-start">
              ...
              >
                ...
                <ul className="mt-4 space-y-1">
                  {([['giris', L.tocSummary], ['hizmet-kapsami', L.tocScope], ['benzersiz', L.tocUnique], ['surec', L.tocProcess]] as const).map(([id, label]) => (
                    <li key={id}>
                      <a
                        href={`#${id}`}
              ...
              <div className="mt-6 rounded-3xl border border-slate-200/80 bg-gradient-to-br from-slate-50 to-white p-5 shadow-lg shadow-slate-900/5">
                ...
                <Link to="/sektorel" className="mt-4 block text-center text-xs font-semibold text-primary-700 hover:underline">
                  {L.seeAll}
                </Link>
              </div>
            </aside>

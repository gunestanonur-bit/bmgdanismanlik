...
import { getPageBanner, getSectoralHeroImage } from '../content/selectors'
import { useSiteContent } from '../content/SiteContentContext'
import { SECTORAL_HUB_HERO, SECTORAL_HUB_SECONDARY } from '../data/sectoralPageVisuals'
import { organizationJsonLd } from '../lib/seo'

export function SectoralHomePage() {
  const { content } = useSiteContent()
  const { site, sectoralServices } = content
  const hubHero = content.visuals.sectoral.hubHero.trim() || SECTORAL_HUB_HERO
  const hubSecondary = content.visuals.sectoral.hubSecondary.trim() || SECTORAL_HUB_SECONDARY
  const pageBanner = getPageBanner(content, 'sektorelHub', hubHero)
  const { pathname } = useLocation()
  const title = `Sektörel hizmetler | ${site.name}`
  const hubStats = [
    { label: 'Odak', value: 'Sektörel', sub: 'havacılık · güvenlik · uyum' },
    { label: 'Hizmetler', value: '4', sub: 'özel program' },
    { label: 'Model', value: 'Entegre', sub: 'eğitim · danışmanlık · belge' },
  ]
  const features = [
    {
      title: 'Sektör gerçekliği',
      text: 'Havacılık, EYDEP ve tesis güvenliği gibi alanlarda mevzuat ve müşteri şartlarıyla uyumlu çalışma.',
    },
    {
      title: 'Tek elden koordinasyon',
      text: 'Analiz, başvuru ve belgelendirme adımlarında net rol ve zaman planı.',
    },
    {
      title: 'Ölçülebilir çıktı',
      text: 'Gap raporu, başvuru paketi veya iyileştirme listesi gibi somut teslimler.',
    },
  ]
  const metaDesc =
    'AS 9100, EYDEP, mevcut durum analizi ve tesis güvenlik belgesi sektörel danışmanlık hizmetleri.'
  ...
  return (
    <>
      ...
      <section className="relative min-h-[min(72vh,40rem)] overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex min-h-[min(72vh,40rem)] max-w-6xl flex-col justify-end px-4 pb-16 pt-28 sm:px-6 sm:pb-20 sm:pt-32 lg:px-8 lg:pb-24">
          <nav className="mb-8 text-sm text-white/70" aria-label="Sayfa konumu">
            ...
            <span className="text-white">Sektörel hizmetler</span>
          </nav>
          ...
          >
            <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-primary-200">Özel gereksinimler</p>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/90">
              Havacılık ve savunma, EYDEP, mevcut durum analizleri ve tesis güvenlik belgesi süreçlerinde danışmanlık ve entegre destek.
            </p>
          </motion.div>
            ...
            <FadeIn>
              <div className="relative overflow-hidden rounded-3xl shadow-2xl shadow-slate-900/15 ring-1 ring-slate-900/5">
                ...
                <p className="absolute bottom-5 left-5 right-5 text-sm font-medium text-white/95">
                  Operasyonel ve regülasyonel gereksinimleri birlikte ele alıyoruz.
                </p>
              </div>
            </FadeIn>
            <FadeIn>
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-primary-600">Neden sektörel?</p>
      ...
      <section className="relative bg-gradient-to-b from-slate-50 to-white py-16 sm:py-20 lg:py-24" aria-labelledby="sektorel-liste">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="max-w-3xl">
              ...
              <h2 id="sektorel-liste" className="mt-3 font-[family-name:var(--font-display)] text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                Sektörel hizmet başlıklarımız
              </h2>
              <p className="mt-4 text-lg leading-relaxed text-slate-600">
                Aşağıdaki başlıkların detay sayfalarına geçebilir veya doğrudan iletişim formu üzerinden talep oluşturabilirsiniz.
              </p>
            </div>
          </FadeIn>

          <div className="mt-14 grid gap-8 sm:grid-cols-2">
            {sectoralServices.map((item, index) => (
      ...
      <section className="relative overflow-hidden border-t border-slate-200/80 bg-slate-900 py-16 sm:py-20">
        ...
        <div className="relative z-10 mx-auto max-w-3xl px-4 text-center sm:px-6">
          <h2 className="font-[family-name:var(--font-display)] text-2xl font-bold text-white sm:text-3xl">
            Hangi sektörel hizmet size uygun?
          </h2>
          <p className="mt-4 text-white/85">
            Kısa bir ön görüşmede kapsamı ve süreyi netleştirelim.
          </p>
